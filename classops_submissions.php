...
try {
    ...
    if ($action === 'grade') {
        ...
        $grade = $_POST['grade'] ?? '';
        $feedback = $_POST['feedback'] ?? ''; // Saved as 'remarks'
        ...
        if ($checkSubStmt->num_rows > 0) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $fileName, $fileType, $fileData, $remarks, $postId, $auth['id']);
        } else {
            // Create new submission
            $sql = "INSERT INTO classops_submissions 
                    (post_id, student_usn, submission_file_name, 
                     submission_file_type, submission_file_data, submitted_at, remarks)
                    VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP(), ?)";

            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isssss", $postId, $auth['id'], $fileName, $fileType, $fileData, $remarks);
        }
...
$conn->close();
ob_end_clean(); // Clean accidental output before returning JSON
header('Content-Type: application/json');
