...
// Fetch user data (name and employee_id) from the hod table
$query = "SELECT name, employee_id FROM hod WHERE email = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("s", $email);
$stmt->execute();
$stmt->bind_result($name, $employee_id);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate email
    $stmt = $conn->prepare("SELECT email FROM hod WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
    ...
    if (empty($emailError) && empty($newPasswordError) && empty($confirmPasswordError)) {
        ...
        $updateStmt = $conn->prepare("UPDATE hod SET password = ? WHERE email = ?");
...
<body>
    ...
    <script>
        ...
        function closeModal() {
            ...
            window.location.href = '<?= BASE_URL ?>app/modules/dashboards/hodDashboard.php'; // Redirect to HOD dashboard after success
        }
