...
// Fetch user data (name and employee_id) from the faculty table
$query = "SELECT name, employee_id FROM faculty WHERE email = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($emailError) && empty($newPasswordError) && empty($confirmPasswordError)) {
        ...
        $updateStmt = $conn->prepare("UPDATE faculty SET password = ? WHERE email = ?");
...
<body>
    ...
    <script>
        ...
        function closeModal() {
            ...
            window.location.href = '<?= BASE_URL ?>app/modules/dashboards/ftDashboard.php'; // Redirect to faculty dashboard after success
        }
